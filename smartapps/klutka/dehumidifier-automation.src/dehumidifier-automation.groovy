 ...
 *
 *
 *		Updated by Justin Klutka to reverse logic for use with dehumidifiers
 *
 */
definition(
    name: "Dehumidifier Automation",
    namespace: "klutka",
    author: "Sheikh Dawood | Justin Klutka",
	...
	if (currentHumidity >= humidityHigh1) {
		...
		if (alreadySentSms1) {
			log.debug "Notification already sent within the last $deltaMinutes minutes"
         	...
         	if (state.lastStatus != "on") {
                log.debug "Humidity Rose Above $humidityHigh1:  sending SMS to $phone1 and turning on $mySwitch"
		...
		if (alreadySentSms2) {
        	...
        	if (state.lastStatus != "off") {
                log.debug "Humidity Dropped Below $humidityLow1:  sending SMS to $phone1 and turning off $mySwitch"
